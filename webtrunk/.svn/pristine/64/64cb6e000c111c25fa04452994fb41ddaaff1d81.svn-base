...
$(function () {
    var setUnitArr=["outputCurrentOfRecitifierMax","outputCurrentOfRecitifierMin","outputCurrentOfRecitifierAvg","outputVoltageOfRecitifierMax","outputVoltageOfRecitifierMin","outputVoltageOfRecitifierAvg","cpLoopResistance","setupPotentialOfRecitifier","setupOffPotentialOfRecitifier","onPotentialOnAnodeAvg","offPotentialOnAnodeAvg","apparentResistanceOfGroundbed","onPotentialOnPipeConnectionAvg","offPotentialOnPipeConnectionAvg","apparentResistanceOfPl"];
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == "1"&&result.detectionData!=null) {
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('查看检测数据', { '结果': '成功', '任务类型': 'M9' });
                    }
                } catch (error) {
                ...
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('查看检测数据', { '结果': '失败', '任务类型': 'M9' });
                    }
                } catch (error) {
