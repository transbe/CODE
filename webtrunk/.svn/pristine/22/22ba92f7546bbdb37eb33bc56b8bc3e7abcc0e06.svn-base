...
var enterprisedObj = {
    ...
    subBusinessmitImgSycn: function(_picid) {
        var that = this;
        $.ajaxFileUpload({
            url: "/cloudlink-core-file/attachment/save?businessId=" + that.userBo.enterpriseId + "&bizType=pic_business&token="+ lsObj.getLocalStorage("token"),
            /*这是处理文件上传的servlet*/
            secureuri: false,
    ...
    submitIdentifySycn: function(_picid) {
        var that = this;
        $.ajaxFileUpload({
            url: "/cloudlink-core-file/attachment/save?businessId=" + that.userBo.enterpriseId + "&bizType=pic_identity&token="+ lsObj.getLocalStorage("token"),
            secureuri: false,
            fileElementId: _picid, //上传input的id
            dataType: "json",
            type: "POST",
            ...
            success: function(data) {
                ...
                if (data.success == 1) {
                    ...
                    for (var i = 0; i < data.rows.length; i++) {
                        var path = "/cloudlink-core-file/file/getImageBySize?fileId=" + data.rows[i].fileId + "&viewModel=fill&width=120&hight=82" + "&token="+ lsObj.getLocalStorage("token");
                        imagesL += '<li><img src="' + path + ' " data-original="/cloudlink-core-file/file/downLoad?fileId=' + data.rows[i].fileId + '&token='+ lsObj.getLocalStorage("token") + ' id="imagesBusiness' + i + '" onclick="previewPicture(this)" alt=""/></li>';
            ...
            success: function(data) {
                if (data.success == 1) {
                    ...
                    if (data.rows.length == 0) {
                        ...
                        for (var i = 0; i < data.rows.length; i++) {
                            var path = "/cloudlink-core-file/file/getImageBySize?fileId=" + data.rows[i].fileId + "&viewModel=fill&width=120&hight=82"+ "&token="+ lsObj.getLocalStorage("token");
                            imagesL += '<li><img src="' + path + '" data-original="/cloudlink-core-file/file/downLoad?fileId=' + data.rows[i].fileId + '&token='+ lsObj.getLocalStorage("token") +' id="imagesIdentify' + i + '" onclick="previewPicture(this)" alt=""/><li>';
