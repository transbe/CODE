...
function  getPhoto(){
    ...
    $.ajax({
        url:'/cloudlink-core-file/attachment/getFileIdListByBizIdAndBizAttr?businessId='+objectId+'&bizType=pic&token=' + lsObj.getLocalStorage("token"),
        method:'get',
        dataType: 'json',
...
var s = "<li class='portrait' style= 'display:inline;padding-right:1% ' ><img class='pic' src='"+rootPath+"\/cloudlink-core-file\/file\/downLoad?fileId="+fileId+ "&token="+ lsObj.getLocalStorage("token") + "' width='120' height='120' data-src-wide='"+rootPath+"\/cloudlink-core-file\/file\/downLoad?fileId="+fileId+"&token="+ lsObj.getLocalStorage("token") + "' width='120' height='120'  alt='BINGOO'/></li>";
document.getElementById("viewPhoto").innerHTML += s
var imageView = "<img id= 'bigPicture' src='"+rootPath+"\/cloudlink-core-file\/file\/downLoad?fileId="+fileId+ "&token="+ lsObj.getLocalStorage("token")+ "' alt='BINGOO' />"
