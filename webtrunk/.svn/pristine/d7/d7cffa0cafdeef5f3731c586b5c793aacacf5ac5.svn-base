...
$(function () {
    ...
    $("#minDetectTime").datetimepicker('setStartDate', '2017-12-26');
    $("#minDetectTime").datetimepicker('setEndDate', '2017-12-28');
    getTaskInfo() //渲染任务页面
    ...
    Option1 = {
        ...
        columns: [
            [{
                ...
                rowspan: 2,
                formatter: function (value, row, index) {
                   var res = "<i class='glyphicon glyphicon-eye-open' title=' "  + getLanguageValue("view") +"'  onclick=\"viewDetectedData('" + row.objectId + '\',\'' + row.detectStatus +"')\"></i>";
                    return res;
                }
            }],
            [{
        ...
        columns: [
            [{
               ...
               title: getLanguageValue("OFFPotentialOFCoupon"),
                ...
                rowspan: 2,
                formatter: function (value, row, index) {
                    var res = "<i class='glyphicon glyphicon-eye-open' title=' "  + getLanguageValue("view") +"'  onclick=\"viewDetectedData('" + row.objectId + "')\"></i>";
                    return res;
                }
            }],
            [{
    ...
    $('#tb-all-task').bootstrapTable({
        ...
        columns: [
            ...
            [{
                ...
                field: 'offPotentialOfCouponAvg',
                title: getLanguageValue("couponOFFPotentialAvg") + getUnit("offPotentialOfCouponAvg"),
                sortable: true,
                width: '5%'
            } ,{
...
function responseHandler(res) {
    if (res.success == 1) {
        try {
            if (tjSwitch == 1) {
                tjSdk.track('查询检测数据', { '任务类型': 'M' + detectMethod, "结果": "成功" });
            }
        } catch (error) {
        ...
        try {
            if (tjSwitch == 1) {
                tjSdk.track('查询检测数据', { '任务类型': 'M' + detectMethod, "结果": "失败" });
            }
        } catch (error) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == 1) {
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('查看任务', { '任务类型': 'M' + detectMethod, "结果": "成功" });
                    }
                } catch (error) {
                ...
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('查看任务', { '任务类型': 'M' + detectMethod, "结果": "失败" });
                    }
                } catch (error) {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success == 1) {
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track(hintMsg, { '任务类型': 'M' + detectMethod, "结果": "成功" });
                    }
                } catch (error) {
                ...
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track(hintMsg, { '任务类型': 'M' + detectMethod, "结果": "失败" });
                    }
                } catch (error) {
...
function exportSelect() {
    ...
    try {
        if (tjSwitch == 1) {
            tjSdk.track('导出选中检测数据', { '任务类型': 'M' + detectMethod });
        }
    } catch (error) {
...
function exportAll() {
    ...
    try {
        if (tjSwitch == 1) {
            tjSdk.track('导出全部检测数据', { '任务类型': 'M' + detectMethod });
        }
    } catch (error) {
