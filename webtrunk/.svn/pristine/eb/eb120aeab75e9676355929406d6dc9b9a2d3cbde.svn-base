	...
	$.ajax({
		...
		success: function (result) {
			if (result.success == 1) {
				...
				if (data.isHighVoltageCorridor == 1) {
					$("#isHighVoltageCorridor").attr("checked", "checked");
					$('#isHighVoltageCorridor').val(1)
				} else {
					$('#isHighVoltageCorridor').val(0)
				}
				if (data.isSubwayParallel == 1) {
					$("#isSubwayParallel").attr("checked", "checked");
					$('#isSubwayParallel').val(1)
				} else {
					$('#isSubwayParallel').val(0)
				}
				...
				if (data.isHighConsequenceArea == 1) {
					$("#isHighConsequenceArea").attr("checked", "checked");
					$('#isHighConsequenceArea').val(1)
				} else {
					$('#isHighConsequenceArea').val(0)
				}
				$("#mileage").val(data.mileage);

				//$("#coordinatesystem").val(data.coordinatesystem);
				if(isNull(data.coordinateSystem)){
					$("input[type=radio][name='coordinateSystem'][value=WGS84]").attr("checked", 'checked');
				}else{
