...
$(function () {
    ...
    // //改变 pickListResult 下拉框的值时进行重新校验    现在没有下拉框
...
function  loadDataDetection(){
    $.ajax({
        url: '/cloudlink-corrosionengineer/task/getDetectField?token=' + lsObj.getLocalStorage("token") +"&detectMethod="+ detectMethod,
        dataType: 'json',
        type: 'get',
...
function renderPage(result){
    ...
    if(result.length>0){
        for(var i=0;i<result.length;i++){
         ...
         $("#dataDetectionList").append("<span style='display:inline-bolck;text-align:center;'>该任务没有数据检测项</span>");
    }
