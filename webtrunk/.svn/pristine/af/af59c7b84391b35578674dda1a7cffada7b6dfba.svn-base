        ...
        success: function (result) {
            if (result.success == 1) {
                ...
                if (data.isHighVoltageCorridor == 1) {
                    environment += ',' + getLanguageValue("isHighVoltageCorridor");
                }
                if (data.isSubwayParallel == 1) {
                    environment += ',' + getLanguageValue("isSubwayParallel");
                }
                if (data.isCrossParallelArea == 1) {
                    environment += ',' + getLanguageValue("Parallel");
                }
                if (data.isHighConsequenceArea == 1) {
                    environment += ',' + getLanguageValue("isHighConsequenceArea");
                }
                var coordinateSystem = data.coordinateSystem?data.coordinateSystem:"WGS84"
                $('#createUser').html(data.createUserName)
                $('#createTime').html(data.createTime)
                $('#type').html(ss.slice(1));
                $('#environment').html(environment.slice(1));
                $("#mileage").html(data.mileage);
                $("#coordinateSystem").html(coordinateSystem);
                $("#x").html(data.x);
                $("#y").html(data.y);
