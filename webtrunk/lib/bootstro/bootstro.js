...
$(document).ready(function() {
    var display =$('#addTaskFrom #importData').css('display');
    if(display == 'none'){
        $('#addTaskFrom #importData').removeClass("bootstro");
    }
    ...
    (function(bootstro, $, undefined) {
        ...
        var nextBtn = '下一步',
            preBtn = '上一步',
            finishBtn = '<i class="icon-ok"></i>关闭';
        if(var_Language == "en"){
            nextBtn = 'Next';
            preBtn = 'Last';
            finishBtn = '<i class="icon-ok"></i>Close';
        }
        var defaults = {
            nextButtonText: nextBtn, //will be wrapped with button as below
            //nextButton : '<button class="btn btn-primary btn-xs bootstro-next-btn">Next &raquo;</button>',
            prevButtonText: preBtn,
            //prevButton : '<button class="btn btn-primary btn-xs bootstro-prev-btn">&laquo; Prev</button>',
            finishButtonText: finishBtn,
            ...
            margin: 30, //if the currently shown element's margin is less than this value
        ...
        function add_nav_btn(content, i) {
            ...
            if (bootstrapVersion == 2)
                defaultBtnClass = "btn btn-mini";
            else
                defaultBtnClass = "btn  btn-xs"; //default bootstrap version 3
            ...
            if (count != 1) {
                if (i == 0)
                    ButtonGroup = finishButton + nextButton;
                else if (i == count - 1)
                    ButtonGroup = finishButton + prevButton;
                else
                    ButtonGroup = finishButton + nextButton + prevButton
            } else {
                ButtonGroup = finishButton
            }
            // if (count != 1) {
            //     if (i == 0)
            ...
            content = content + '<div class="bootstro-finish-btn-wrapper">' + ButtonGroup + '</div>';
            return content;
        }
        ...
        get_popup = function(i) {
            ...
            //p.selector = selector;
            var t = '';
            var hintPic = "<span class='hint-pic'> </span>";
            if (count > 1) {
            ...
            if (p.title != '' && t != '')
            // p.title = t + ' - ' + p.title;
                p.title = hintPic + p.title + ' -- ' + t;
            else if (p.title == '') {
                p.title = hintPic + t;
            } else {
                p.title = hintPic + p.title;
            }
            ...
            if ($el.attr('data-bootstro-margin-left')) {
                p.left = $el.attr('data-bootstro-margin-left');
                left = p.left;
                style = style + 'margin-left:' + $el.attr('data-bootstro-margin-left') + ';'
            }
            if ($el.attr('data-bootstro-margin-top')) {
                p.top = $el.attr('data-bootstro-margin-top');
                style = style + 'margin-top:' + $el.attr('data-bootstro-margin-top') + ';'
            }
            ...
            p.template = '<div class="popover" style="' + style + '"><div class="arrow" style="margin-left:-' + left + '"></div><div class="popover-inner"><h3 class="popover-title"></h3><div class="popover-content"><p></p></div></div>' +
                '</div>';
