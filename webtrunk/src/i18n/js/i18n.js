...
function ReplaceStaticText() {
    ...
    try {
        ...
        for (var i = 0; i < palt.length; i++) {
            ...
            /**********************  首页页面跳转的国际化 ********************** */
            var spankey1 = $(palt[i]).attr("key1");
            if(!isNull(spankey1)){
                var arrRes1 = Enumerable.From(jsonData).Where("x=>x.key1=='" + spankey1 + "'").ToArray(); //使用linq查询返回json中的value，只需要第一个
                if(arrRes1 && arrRes1.length>0){
                    var valuestr1 = arrRes1[0].value;
                    var dataText = palt[i].getAttribute("data-text"),
                        dataSearchId = palt[i].getAttribute("data-searchId");
                    if(dataText != undefined&&dataText != null&&dataText != ""&& dataSearchId != undefined&&dataSearchId != null&&dataSearchId != ""){
                         palt[i].setAttribute("data-text",valuestr1);
                    }
                }
            }
            /*********************************************** */
