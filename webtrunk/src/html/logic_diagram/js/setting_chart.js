...
function setShapeAttrDefaultValue(ft) {
    ...
    if (ft == "FACILITY_STATION") { //站场0
        ...
        if (defaultIndex[5] == 1 && defaultText.tee == undefined) {
            ...
            $("#attributeTable input[name='text']").val(_Tee + "_001");
        } else {
    ...
    if (pageMeta.selectedBean == shape) {
        ...
        for (var a = 0; a < list.length; a++) {
            ...
            if (input.name == 'defaultText') {
                ...
                var defaultTr = '<tr style="line-height:20px;margin-bottom:5px;"><td colspan="2" style="text-align:left;"><input type="radio" name="' + input.name + '"  id="radioBtn" style="margin: 0px 5px 0px 10px"/><lable class="CP-i18n" key="Set_as_default_attribute" for="radioBtn" style="display: table-cell;width: 140px;float: right;margin-top: -2px;">' + getLanguageValue("Set_as_default_attribute") + '</lable></td></tr>';
...
function setDefaultText(ft) {
    if (ft == "FACILITY_STATION") { //站场
        defaultText.station = {
            "name": $("#attributeTable input[name='text']").val() || ""
        };
        if (firstTime == true) {
        ...
        defaultText.linePE = {
            "name": $("#attributeTable input[name='text']").val() || "",
            "innerStation": $("#attributeTable select[name='innerStation']").val() || 0,
            "length_": $("#attributeTable input[name='length']").val() || "",
            "diameter": $("#attributeTable input[name='diameter']").val() || "",
            "wallThickness": $("#attributeTable input[name='wallThickness']").val() || ""
        }
        if (firstTime == true) {
        ...
        defaultText.rectifier = {
            "name": $("#attributeTable input[name='text']").val() || "",
            "specifications": $("#attributeTable input[name='specifications']").val() || ""
        };
        if (firstTime == true) {
        ...
        defaultText.spd = {
            "name": $("#attributeTable input[name='text']").val() || "",
            "installType": $("#attributeTable select[name='installType']").val() || "地上"
        };
        if (firstTime == true) {
...
function previousView() {
    ...
    if (arr != null && arr.length > 0) {
        if (arr[0].facilityType == "FACILITY_RECTIFIER") {
            var idshd = arr[0].id;
            var index = parent.layer.confirm(getLanguageValue("Revoke_test_pile_is_deleted"), {
                title: getLanguageValue("tip"),
                btn: [getLanguageValue("OK"), getLanguageValue("cancel")], //按钮 //按钮
                skin: 'self',
                yes: function() {
        ...
        if (arr[0].facilityType == "LINE_3PE") {
            var idspe = arr[0].id;
            var index = parent.layer.confirm(getLanguageValue("Revoke_associated_test_pile"), {
                title: getLanguageValue("tip"),
                btn: [getLanguageValue("OK"), getLanguageValue("cancel")], //按钮 //按钮
                skin: 'self',
                yes: function() {
...
function clicksave() {
    ...
    saveChartForLog(attr,defaultIndex);
}
