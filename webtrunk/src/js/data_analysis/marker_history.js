    ...
    $(".suspension").click(function() {
        if (this.outerText == getLanguageValue("history.pre")) {
            if (currentNode.preNode != null) {
                ...
                setMarkerType(currentNode.preNode);
            ...
            if (currentNode.nextNode != null) {
                ...
                setMarkerType(currentNode.nextNode);
...
function setMarkerType(markerId){
    var markType = "";
    var data = Enumerable.From(markerList).Where("x=>x.id=='" + markerId + "'").ToArray(); 
    
    if (data[0].isDrivepipe == 1) {
        markType += ','+getLanguageValue("Casing");
    }
    if (data[0].isInsulatedJoint == 1) {
        markType += ','+getLanguageValue("Insulating");
    }
    if (data[0].isDirectionalDrilling == 1) {
        markType += ','+getLanguageValue("Drainage");
    }
    if (data[0].isRecitifierNearest == 1) {
        markType += ','+getLanguageValue("HDD");
    }
    if (data[0].isDrainageAnode == 1) {
        markType += ','+getLanguageValue("Drain");
    }
    markType = markType.substring(1);
    if((markType.length>20&&var_Language=="zh")||(markType.length>36&&var_Language=="en")){
        $("#markerType").removeClass("markerTypeLine");
    }else{
        $("#markerType").addClass("markerTypeLine");
    }
    $("#markerType").text(markType);
}
/**
 *  @desc 动态获取曲线图盒子的高度
 */
...
function initEcharts() {
    ...
    option = {
        ...
        legend: {
            right: "3%",
            data: ["测试桩类型",getLanguageValue("field.AC"), getLanguageValue("field.AC.density")] //交流电压 交流电流密度
        },
        dataZoom: [{
            type: 'slider',
            bottom: 45,
    ...
    myChart1.on('click', function(params) {
        var detectMethod = detectMethods[params.dataIndex];
        var objectId = businessIds[params.dataIndex];
        var content= getRootPath() + "/../src/html/task/specific_task/view_detectedData_m"+detectMethod+".html?detectMethod="+detectMethod+"&id="
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success == 1) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        businessIds.push(data[i].objectId);
                        detectMethods.push(data[i].detectMethod);
...
var M4_columns = [
    [{
        ...
        },
        {
            ...
            title: getLanguageValue("field.CasingOnPotential") + '(mV)',
            sortable: true,
            width: '11%',
            rowspan: 2
        }, {
            field: 'offPotenialOfCasing',
            title: getLanguageValue("field.CasingOffPotential") + '(mV)',
            sortable: true,
            width: '11%',
            rowspan: 2
        },
