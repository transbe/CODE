...
function showData() {
    ...
    for (var i = 0; i < markerNumber.length; i++) {
        ...
        str += "<tr><td>" + (i + 1) + "</td><td>" + markerNumber[i] + "</td><td>" + pipelineName[i] + "</td><td style = 'width:300px;'><input type='text' value='' style='width:100%;height:30px;'/></td></tr>"
    }
    $('#rejected').append(str);
}
...
function submitData() {
    ...
    for (var i = 0; i < markerNumber.length; i++) {
        ...
        // }
        if (i != objectID.length - 1) {
            data += '{"objectId":"' + objectID[i] + '", "taskId": "' + taskID[i] + '", "markerId":"' + markerID[i] + '","remark":"' + $("#rejected tr").eq(i + 1).find("td").eq(3).find("input").val() + '"},';
        } else {
            data += '{"objectId":"' + objectID[i] + '", "taskId": "' + taskID[i] + '", "markerId":"' + markerID[i] + '","remark":"' + $("#rejected tr").eq(i + 1).find("td").eq(3).find("input").val() + '"}';
        }
    }
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success == 1) {
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('驳回检测数据', { '任务类型': 'M' + detectMethod, "结果": "成功" });
                    }
                } catch (error) {
                ...
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('驳回检测数据', { '任务类型': 'M' + detectMethod, "结果": "失败" });
                    }
                } catch (error) {
