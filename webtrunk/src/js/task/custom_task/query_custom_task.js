...
function loadTable() {
    ...
    var url = handleURL('/cloudlink-corrosionengineer/task/customTask');  //对url进行权限处理
    //初始化Table
    $('#tb-all-task').bootstrapTable({
        ...
        // },
         {
            ...
            // formatter: function (value, row, index) {
            //     var createTime = (row.createTime).split(' ');
            //     var showCreateTime = createTime[0];
            //     console.log(showCreateTime);
            //     // console.log(showCreateTime.lastIndexOf("-"));
            //     if(showCreateTime.lastIndexOf("-") == 6){
            //         showCreateTime = insert_flg(showCreateTime,"0",5);
            //     }
            ...
            formatter: function (value, row, index) {
                ...
                // 导入历史数据
                var f = '<a href="#" mce_href="#" title=" ' + getLanguageValue("importHistoricalRecord") +'" onclick="ImportHistoricalRecord(\'' + row.objectId + "\',\'" + row.taskStatus + '\')"><span class="glyphicon glyphicon-import"></span></a> ';

                if (!judgePrivilege()) {  // 非运营人员
                    if (roleNum != 2) {  // 非现场检测人员
                        ...
                        if(row.taskStatus == "2" ||  row.taskStatus == "3"){
                            f = '<a href="#" mce_href="#" title=" ' + getLanguageValue("importHistoricalRecord") +'" onclick="ImportHistoricalRecord(\'' + row.objectId  + "\',\'" + row.taskStatus+ '\')"><span class="glyphicon glyphicon-import" style="color:#ccc"></span></a> ';
                        }
        ...
        responseHandler: function (res) { //加载服务器数据之前的处理程序，可以用来格式化数据。参数：res为从服务器请求到的数据。 
            if (res.success == 1) {
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('查询任务', { '任务类型': 'M' + detectMethod, "结果": "成功" });
                    }
                } catch (error) {
                ...
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('查询任务', { '任务类型': 'M' + detectMethod, "结果": "失败" });
                    }
                } catch (error) {
...
function addTask() {
    ...
    var index = parent.layer.open({
        ...
        content: getRootPath() + "/src/html/task/custom_task/add_custom_task.html?detectMethod=" + detectMethod+"&templateId="+templateId
    });
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success == 1) {
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('删除任务', { '任务类型': 'M' + detectMethod, "结果": "成功" });
                    }
                } catch (error) {
                ...
                try {
                    if (tjSwitch == 1) {
                        tjSdk.track('删除任务', { '任务类型': 'M' + detectMethod, "结果": "失败" });
                    }
                } catch (error) {
...
function viewGraph() {
    ...
    var layer1 = parent.layer.open({
        ...
        content: getRootPath() + "/src/html/task/custom_task/graph_custom_task.html?objectId=" + objectId + '&detectMethod=' + detectMethod + '&taskName=' + encodeURI(taskName)
    });
};
...
function exportSelect() {
    ...
    try {
        if (tjSwitch == 1) {
            tjSdk.track('导出选中任务', { '任务类型': 'M' + detectMethod });
        }
    } catch (error) {
...
function exportAll() {
    ...
    try {
        if (tjSwitch == 1) {
            tjSdk.track('导出全部任务', { '任务类型': 'M' + detectMethod });
        }
    } catch (error) {
...
function view(objectId, taskStatus, detectUserId,taskName,auditorId) {
    ...
    } else if (rowDate.length == 1) {  // 选中一条记录
    ...
    if (!judgePrivilege()) {   // 非运营人员
        if (roleNum == 2) { //登录角色是现场检测人员
            ...
            if (detectUser == detectUserId && taskStatus == "1") {  //任务检测ID与登录人员id一致 && 任务状态待领取 
                ...
                // 提交审核  取消
                btn = [getLanguageValue("submit_btn"), getLanguageValue("cancel")];
            } else if (detectUser == auditorId  && taskStatus == "3") {
    ...
    var index = parent.layer.open({
        ...
        yes: function (index, layero) {
            if(!preventDblClick){
                ...
                if (!judgePrivilege()) {
                    ...
                    } else if ((roleNum != 2 && taskStatus == "3" && detectUser == auditorId)) { 
                        //权限不是现场检测人员且任务状态为待审核  并且当前登录id和审核人员id相同
        ...
        content: getRootPath() + "/src/html/task/custom_task/view_custom_task.html?objectId=" + objectId + '&detectMethod=' + detectMethod + '&taskStatus=' + encodeURI(taskStatus) + '&detectUserId=' + detectUserId + '&taskName='+encodeURI(taskName)+"&templateId="+templateId  //获取检测方法"
    });
    parent.layer.full(index);
};
...
function updateTask(objectId, taskStatus) {
    ...
    if (taskStatusId == "4") {
        ...
        var index = parent.layer.open({
            ...
            btn2: function (index, layero) { },
            content: getRootPath() + "/src/html/task/custom_task/add_custom_task.html?objectId=" + objectId + "&taskStatus=" + taskStatusId + "&detectMethod=" + detectMethod+"&templateId="+templateId
        });
        parent.layer.full(index);
    }
}
...
/**
 * @desc 导入历史记录数据
 * @method ImportHistoricalRecord
 */
function ImportHistoricalRecord(taskId, taskStatus){
    var preventDblClick = false;
    var taskStatusId = taskStatus;
    if (taskStatusId == "2" || taskStatusId == "3") {
        return;
    } else { 
        var index = parent.layer.open({
            type: 2, //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
            title: getLanguageValue("importHistoricalRecord"),
            area: ['500px', '450px'],
            btn: [getLanguageValue("upload_btn"), getLanguageValue("close_btn")], //按钮
            skin: 'self-iframe',
            yes: function (index, layero) {
                ...
                if (!preventDblClick == true) {
                    ...
                    if (result == true) {
                        ...
                        parent.$('.layui-layer-btn1').html(getLanguageValue("close_btn"));
                        // reloadPage();
                        $('#tb-all-task').bootstrapTable('refresh', true);
                    } else {
                        preventDblClick = false;
            ...
            content: rootPath + "/src/html/task/custom_task/custom_task_history.html?taskId=" + taskId + '&templateId=' + templateId 
        });
    }
}
