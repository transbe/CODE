...
$('.btn3').click(function() {
    ...
    if(language == "zh"){
        ...
        if (province == "" || province == null || city == "" || city == null || county == "" || county == null) {
            ...
            $('.addressMsg span').text(getLanguageValue("completeAddress"));
            x = false;
            return;
        } else {
            $('.addressMsg').css({
        ...
        if (detailAddress == "" || detailAddress == null) {
            ...
            y = false;
            $('.detailAddressMsg span').text(getLanguageValue("detailAddressNull"));
            return;
        } 
    }else{
        x = true;
    ...
    if (b == true && c == true && x == true && y == true) {
         if(isExistAccount == true){  // 手机账户已经存在
            _data = {
            ...
            registEnterpriseAndLogin(_data);
        }else{
            _data = {  // 手机账户不存在
                "enterpriseName": enterpriseName,
                "enterpriseScale": Scale,
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success == 1) {
                        ...
                        // dataObj.appCode = appCode;
                        console.log(data.rows[0].enterpriseId);
                        var row = data.rows;
        ...
        $.ajax({
            ...
            success: function(data, status) {
                ...
                if (res == true && data.rows[0].isExist == true) {
                    layer.confirm(getLanguageValue("phoneHasDone"), {
                        title:getLanguageValue("tip_title"),
                        btn: [getLanguageValue("jumpLogin"),getLanguageValue("recreate")],
                        skin: 'self-alert',
                    },function(index){
                        layer.close(index);
                        location.href = 'login.html';
                        ...
                        isExistAccount = true;
                        layer.prompt({
                            title: getLanguageValue("enterPwd"),
                            skin: 'self-alert',
                            formType: 1//密码输入框
                        }, function (val, index1) {
                            loginGetToken(val);
...
$('.province').blur(function() {
    ...
    if (province != getLanguageValue("select")) {
        $('.addressMsg').css({
            display: 'none'
        });
    }
...
$('.city').blur(function() {
    ...
    if (city != getLanguageValue("select")) {
        $('.addressMsg').css({
            display: 'none'
        });
    }
...
$('.county').blur(function() {
    ...
    if (county != getLanguageValue("select")) {
        $('.addressMsg').css({
            display: 'none'
        });
    }
...
/**
 * @desc 通过手机号密码登陆获取token
 */
function loginGetToken(passwordVal){
    var mobileNum = $('.phone').val();
    var _data = {
        "loginNum": mobileNum,
     ...
     $.ajax({
        ...
        success: function (data) {
            if (data.success == 1) {
                tokenVal = data.token;
                lsObj.setLocalStorage('token', tokenVal);
                ...
                layer.alert(getLanguageValue("passwordError"), {
                    title: getLanguageValue("tip_title"),
                    skin: 'self-alert'
                });
            }
 ...
 * @param {*String} _data 
 */
function registEnterpriseAndLogin(data1){
     $.ajax({
        type: "post",
