...
function messageTyppe() {
    $.ajax({
        url: "/cloudlink-corrosionengineer/message/msgTypeCheck?domain=msg_business_type&token="+token,
        dataType: "json",
        method: "get",
...
function loadTable() {
    $('#news').bootstrapTable({
        url: '/cloudlink-corrosionengineer/message/queryAllMessageForPage?token=' + token,
        method: 'get', //请求方式（*）
        toolbar: '#toolbar',
        ...
        columns: [{
            ...
            formatter: function(value, row, index) {
                ...
                var d = '<a href="#"  mce_href="#" title="'+getLanguageValue("field.delete")+'" onclick="deletNews(this,\'' + row.objectId + '\'\,\'' + row.businessType + '\'\,\'' + row.businessDataId + '\')"><span class="glyphicon glyphicon-trash"></span></a> ';
                e = '<a href="#"   mce_href="#" title="'+getLanguageValue("field.view")+'" onclick="oneClick(\'' + row.objectId + '\'\,\'' + row.businessType + '\'\,\'' + row.businessDataId + '\')"><i class="glyphicon glyphicon-eye-open  "></i></span></a> ';
                return e + d;
            }
        }],
