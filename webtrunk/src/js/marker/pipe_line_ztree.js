...
$(window).resize(function() {
    ...
    $(".divider").css("height",window_h-5+"px");
    $(".ztree").css("height",window_h-38-5+"px");
...
function loadTree(token){
       $.ajax({
        ...
        success:function(result){
            ...
            //    try{
            //         if(tjSwitch==1){
            //             tjSdk.track('测试桩管理',{'操作':'查询管线'});
            //         }
            //     }catch(err){
            //         //在此处理错误
            //     }
            }else if(result.success==0){
                layer.msg(result.msg,{skin:"self-msg"});
...
function OnRightClick(event, treeId, treeNode) {
        console.log(event);
        if(treeNode && !judgePrivilege()){
            zTree.selectNode(treeNode);
            // console.log($(".curSelectedNode").offset().left);
            // alert();
            onClick(event, treeId, treeNode);
            var mX = $(".curSelectedNode").offset().left;
            var	mY = $(".curSelectedNode").offset().top+24;
            ...
            $.ajax({
                ...
                success:function(result){
                    ...
                    if(result.success==1){
                        ...
                        // zTree.updateNode(treeNode);
                        layer.msg("删除成功",{skin:"self-msg"});
                        try{
                            if(tjSwitch==1){
                                tjSdk.track('删除管线',{'结果':'成功'});
                            }
                        }catch(err){
                            //在此处理错误
                        }
...
function renameNodeName(){
    var nodes=zTree.getSelectedNodes();
    var treeNode=nodes[0];
    zTree.editName(treeNode);
    $(".curSelectedNode_Edit input").select();
    hideRMenu();
}
// 重命名下拉树节点前触发的操作
function beforeRename(treeId, treeNode, newName, isCancel) {
    ...
    if (newName.length == 0) {
        ...
        flag=false;
        layer.msg("名称不能为空",{skin:"self-msg"});
    ...
    for(var i=0;i<allNodes.length;i++){
        ...
        if(newName==text&&text!=treeNode.name){
            ...
            layer.msg("已存在该名称",{skin:"self-msg"});
    ...
    if(flag){
        $.ajax({
            ...
            success:function(result){
                if(result.success==1){
                    layer.msg("修改名称成功",{skin:"self-msg"});
...
function addLeafNode(treeNode,newname){
    ...
    if(flag){
        ...
        layer.msg("该名称已存在",{skin:"self-msg"});
        ...
        $.ajax({
            ...
            success:function(result){
                if(result.success==1){
                    var id=result.objectid;
                    var zTree = $.fn.zTree.getZTreeObj("treeDemo");
                    zTree.addNodes(0, {id:id, pId:0, name:rootnodename });
                    // zTree.expandAll(true);
                    layer.msg("添加成功",{skin:"self-msg"});
                    query();
