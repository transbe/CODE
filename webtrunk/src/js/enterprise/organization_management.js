...
var organizationObj = {
    ...
    init: function() {
        ...
        this.$addDepart.click(function() {
            if (_this._selectedId == '' || _this._selectedId == null) {
                xxwsWindowObj.xxwsAlert(getLanguageValue("js.please.wantadd"));
            } else {
        ...
        this.$addBtn.click(function() {
            ...
            if (_this._flag == true) {
                ...
                if (addOrgName == null || addOrgName == "") {
                    xxwsWindowObj.xxwsAlert(getLanguageValue("js.please.wantadd.name"));
                    _this.again();
                } else {
        ...
        $(".child_list").on('click', '.delete_btn', function() {
            ...
            var defaultOptions = {
                tip: getLanguageValue("tip.delete"),
                name_title: getLanguageValue("tip"),
                name_cancel: getLanguageValue("html.department.cancle"),
                name_confirm: getLanguageValue("html.department.save"),
        ...
        this.$modifyBtn.click(function() {
            ...
            if (_this._flag == true) {
                ...
                if (orgChildName == null || orgChildName == "") {
                    xxwsWindowObj.xxwsAlert(getLanguageValue("tip.update.departname"));
                    _this.again();
                } else {
                    ...
                    if (childData.length > 0) {
                        for (var i = 0; i < childData.length; i++) {
                            ...
                            txt = '<li data-id="' + childData[i].orgId + '">' +
                                ...
                                '<span data-placement="top" title="'+getValueHasArgs("tip.1",[childData[i].activeUserCount,childData[i].unactiveUserCount,childData[i].frozenUserCount])+'" data-toggle="tooltip" class="children_num"><b>' + allNum + '</b>&nbsp;</span>' +
                                '<i class="delete_btn" title="'+getLanguageValue("tip.delete.confirm")+'"></i>' +
                                '<i class="modify_btn" title="'+getLanguageValue("tip.update.title")+'"></i></li>';
                                
                            // 运营人员进来没有修改与删除功能，直接不加载
                            txt1 = '<li data-id="' + childData[i].orgId + '">' +
                                '<span class="clildren_name">' + childData[i].orgName + '</span>' +
                                '<span data-placement="top" title="'+getValueHasArgs("tip.1",[childData[i].activeUserCount,childData[i].unactiveUserCount,childData[i].frozenUserCount])+'" data-toggle="tooltip" class="children_num"><b>' + allNum + '</b>&nbsp;</span></li>';
                        ...
                        txt = '<li>'+getLanguageValue("tip.nohaveone")+'</li>';
                        _this.$childList.append(txt);
                    }
                    $('[data-toggle="tooltip"]').tooltip();
                } else {
                    xxwsWindowObj.xxwsAlert(getLanguageValue("tip.loadData.fail"));
                }
            },
            error: function() {
                xxwsWindowObj.xxwsAlert(getLanguageValue("tip.loadData.fail"));
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success == 1) {
                    ...
                    xxwsWindowObj.xxwsAlert(getLanguageValue("tip.department.exist"));
                }
                _this.again();
            },
            error: function() {
    ...
    deleteDepartment: function(objId) { //删除部门
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success == 1) {
                    xxwsWindowObj.xxwsAlert(getLanguageValue("tip.delete.success"));
                    _this.getAllData();
                } else {
                    ...
                    if (data.code == "PU02020") {
                        var defaultOptions = {
                            tip: getLanguageValue("tip.exist.org.haspeople"),
                            name_title: getLanguageValue("tip"),
                            name_cancel: getLanguageValue("html.department.cancle"),
                            name_confirm: getLanguageValue("html.department.save"),
                    ...
                    if (data.code == "PU02021") {
                        var defaultOptions = {
                            tip: getLanguageValue("tip.exist.org"),
                            name_title: getLanguageValue("tip"),
                            name_cancel: getLanguageValue("html.department.cancle"),
                            name_confirm: getLanguageValue("html.department.save"),
                            isCancelBtnShow: false,
