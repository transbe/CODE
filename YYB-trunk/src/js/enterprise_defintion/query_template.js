        ...
        $('#taskTemplate').bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    ...
                    if(row.isVisible==0){
                        divClass = "bg_con_off"; 
                        divTitle = getLanguageValue("hideInMenu");
                    }else{
                        divTitle = getLanguageValue("showInMenu");
                    }
                    ...
                    var f = '<div class="switch_container" title = "'+divTitle+'" onchange="showOrhide(\'' + row.objectId + '\',\'' + row.isVisible + '\',\'' + index + '\',this)">'+  
...
//是否在任务管理列表中显示
function showOrhide(objectId,isVisible,index,obj){
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success == 1) {
                ...
                parent.parent.loadSelfDefinedTemplate();
                try {
                    if (zhugeSwitch == 1) {
                        zhuge.track('设置任务是否可见', {
                            '操作': '成功'
                        });
                    }
                } catch (error) {
                ...
                parent.parent.layer.msg(getLanguageValue("set-success"), {
                ...
                parent.parent.layer.alert(res.msg, {
                    title: getLanguageValue("tip_title"),
                    skin: 'self-alert'
                });
            }
...
function updateTemplate(objectId){
    ...
    if(!validateTemplate(objectId)){
        parent.parent.layer.alert(getLanguageValue("updateTips"), {
            title: getLanguageValue("tip_title"),
            skin: 'self-alert'
        });
        return;
...
function deleteTemplate(){
    ...
    if (rowDate.length == 1) {
        if(!validateTemplate(rowDate[0].objectId)){
            parent.parent.layer.alert(getLanguageValue("deleteTips"), {
                title: getLanguageValue("tip_title"),
                skin: 'self-alert'
            });
            return;
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success == 1) {
                ...
                parent.parent.loadSelfDefinedTemplate();
                parent.parent.layer.msg(getLanguageValue("delete_success"), {
                    time: MSG_DISPLAY_TIME,
                    skin: "self-msg"
                });
            } else {
