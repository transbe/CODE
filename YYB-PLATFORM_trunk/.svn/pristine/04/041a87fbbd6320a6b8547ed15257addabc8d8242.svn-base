              ...
              $.ajax({
                  ...
                  success: function(result) {
                      if (result.success == 1) {
                          ...
                          // 原始头部数据加载
                          //   $("#cpsegment").html(data.pipeTotal.pipeSum + "条");
                          //   $("#pipeSum").html(data.pipeTotal.cpsegment + "个");
                          //   $("#yearOfTask").html(data.yearOfTask.totalTaskCount + "个");
                          //   $("#totalOfMark").html(data.totalOfMark.total + "个");
                          ...
                          // 画本年度任务图表
                          //   setYearsGraph();
                          //   drowYearTaskGraph(data.yearOfTask);
  ...
  function setMarkType() {
      ...
      //   ResultChart = echarts.init(document.getElementById('result-chart'));
      ...
      // 使用刚指定的配置项和数据显示图表。
      //   MarkOption = ResultOption = StausOption;
      MarkChart.setOption(MarkOption, true);
      //   ResultChart.setOption(MarkOption, true);
  ...
  function drowMarkTypeGraph(data) {
      ...
      //   var evaluateResultY = ["高腐蚀风险区", "低腐蚀风险区", "交流干扰区", "直流干扰区", "交直流干扰区"];
      //   evaluateResultArr.push(evaluateResult.hightCorrosion);
      //   evaluateResultArr.push(evaluateResult.lowCorrosion);
      //   evaluateResultArr.push(evaluateResult.acRegion);
      //   evaluateResultArr.push(evaluateResult.dcRegion);
      //   evaluateResultArr.push(evaluateResult.acdcRegion);
      //   //   console.log(evaluateResultArr);
      //   if (ResultOption && typeof ResultOption === "object") {
      //       ResultOption = MarkOption;
      //       ResultOption.series[0].data = evaluateResultArr;
      //       ResultOption.yAxis[0].data = evaluateResultY;
      //       ResultChart.setOption(ResultOption, true);
      //   }
      ...
      //console.log(totalOfMarkStausArr);
