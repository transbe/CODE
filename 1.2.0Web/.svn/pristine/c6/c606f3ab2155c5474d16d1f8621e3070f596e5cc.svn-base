  ...
  * @last modified time: 2017-05-04 15:22:36
  * @file:专家页面首页配置
  */
    ...
    $.ajax({
        ...
        success:function(result){
            if(result.success == 1){
                ...
                var effectiveGaugeData = [{value: data.effectiveReportAverageDays, name: '天/份'}];
                var integrityGaugeData = [{value: data.integrityReportAverageDays, name: '天/份'}];
    ...
    $.ajax({
        ...
        success:function(result){
            if(result.success == 1){
                ...
                var reportData = [
                    {value:effectiveCount, name:'有效性报告'},
                    {value:integrityCount, name:'完整性报告'}
                ];
                var auditData = [
                    {value:finishReport, name:'审核完成',itemStyle: {normal: {color: '#4fc877'}}},
                    {value:auditingReport, name:'审核中',itemStyle: {normal: {color: '#f8a950'}}},
                    {value:otherReport, name:'其他',itemStyle: {normal: {color: '#fa765e'}}}
                ];
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == 1) {
                ...
                layer.msg("加载数据失败", { time: MSG_DISPLAY_TIME,skin: "self-success" });
            }
        }
    });
}
