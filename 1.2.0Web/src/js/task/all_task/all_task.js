...
var query = getParameter("query"); //获取token
 ...
 * @desc 查看任务详情
 * @method viewTask
 * @param {*String} objectId, detectMethod, taskStatus, detectUserId,taskName
 */
...
function exportAll() {
    ...
    var url = '/cloudlink-corrosionengineer/task/exportTask?token=' + token + '&pipeID=' + $("#pipeName").val() + '&taskName=' + $("#taskName").val() +
        '&detectMethed=' + getCheckVal() + '&year=' + $("#year").val() + '&detectUserId=' + $("#detectUserName").val() + '&taskStatus=' + $("#taskStatus").val() + '&type=query';
    $("#exprotExcelIframe").attr("src", url);
    try {
        if (zhugeSwitch == 1) {
